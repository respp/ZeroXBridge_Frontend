...
const SubmitTicketCard: React.FC<SubmitTicketCardProps> = ({ className = '', onClick }) => {
  const handleClick = () => {
    if (onClick) {
      onClick();
    } else {
  ...
  return (
    ...
    >
      ...
      {/* Decorative illustration - Ticket image */}
      <div className="absolute top-[55px] -left-[8px] w-[430px] h-[430px] pointer-events-none">
        <div className="w-full h-full opacity-70 group-hover:opacity-80 transition-all duration-300 flex items-center justify-center">
          <Image
            src="/ticket.png"
            alt="Submit a ticket illustration"
            width={430}
            height={430}
